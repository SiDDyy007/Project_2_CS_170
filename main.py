...
def feature_search_demo(num_features):
    ...
    choice = int(input())
    if choice == 1:
        forward_selection(data, current_set_of_features)
    else:
        backward_elimination(data, current_set_of_features)
    
def forward_selection(data, current_set_of_features):
...
def backward_elimination(data, current_set_of_features):
    # Using all features and “random” evaluation
    accuracy = dummy_function(data, current_set_of_features, 0)
    print(f"Using all features and “random” evaluation, I get an accuracy of {accuracy * 100}%")
    print("Beginning search.")
    ...
    for i in range(data.shape[1] - 1):
        ...
        for k in range(data.shape[1] - 1):
            if k in current_set_of_features:
                print(f'Considering removing the {k + 1} feature')
                current_set_of_features.remove(k)
                accuracy = dummy_function(data, current_set_of_features, k)
                print(f'Using feature(s) {current_set_of_features} accuracy is {accuracy * 100}%')
                if accuracy > best_so_far_accuracy:
